...
struct ThemeDemoView: View {
    ...
    @State private var selected: AppTheme = .system
    ...
    var body: some View {
        ...
        .onAppear {
            tm.apply(selected, colorScheme: cs)
        }
    }
    ...
    private var headerSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("SkinBro")
                ...
                .foregroundColor(tm.theme.palette.textPrimary)
            Text("Erkeklere özel, 3 adımda günlük cilt rutini.")
    ...
    private var progressSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    Circle().fill(tm.theme.palette.secondary).frame(width: 10, height: 10)
                    Text("Sabah rutini tamamlandı")
                        .font(tm.theme.typo.body)
                        .foregroundColor(tm.theme.palette.textPrimary)
                }
                Divider().background(tm.theme.palette.separator)
                HStack {
                    Circle().fill(tm.theme.palette.accent).frame(width: 10, height: 10)
                    Text("Akşam SPF hatırlatıcısı açık")
                        .font(tm.theme.typo.body)
                        .foregroundColor(tm.theme.palette.textPrimary)
                }
            }
