...
struct MorningRoutineCompletionView: View {
    ...
    var body: some View {
                ...
                do {
                    ...
                    if let routine = activeRoutine {
                        ...
                        if !cycleAlreadyEnabled && premiumManager.isPremium && hasCycleData {
                            print("🔄 [MorningCompletion] Auto-enabling cycle adaptation for premium user with cycle data")
                            ...
                            activeRoutine = try await routineStore.fetchActiveRoutine()
                            print("✅ [MorningCompletion] Auto-enabled cycle adaptation")
                        }
                    }
        ...
        .fullScreenCover(item: $companionLaunch) { launch in
            CompanionSessionView(
                ...
                routineName: L10n.Routines.morningRoutine,
                steps: launch.steps,
        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                VStack(spacing: 16) {
                    ...
                    HStack {
                        Text(L10n.Routines.morningRoutine.uppercased())
    ...
    private func updateRoutineStepsSync(from routine: RoutineResponse, oldCompletedSteps: Set<String>) {
        ...
        if let activeRoutine = activeRoutine {
            let morningStepsFromSaved = activeRoutine.stepDetails.filter { $0.timeOfDay == "morning" }.sorted { $0.order < $1.order }
            routineSteps = morningStepsFromSaved.map { stepDetail in
                RoutineStepDetail(
            ...
            routineSteps = routine.routine.morning.enumerated().map { (index, apiStep) in
                RoutineStepDetail(
                    id: "morning_\(apiStep.step.rawValue)_\(index)",
                    title: apiStep.name,
    ...
    private func performEnableCycleTracking() async {
        ...
        if cycleAlreadyEnabled {
            print("ℹ️ [MorningCompletion] Cycle adaptation already enabled, skipping update")
            await MainActor.run {
                withAnimation {
                    showCyclePromotion = false
                }
        ...
        do {
            print("🔄 [MorningCompletion] Enabling cycle adaptation for routine \(routine.id)")
            try await routineStore.updateAdaptationSettings(
                routineId: routine.id,
                enabled: true,
                type: .cycle
            ...
            do {
                ...
                if let updated = activeRoutine {
                    ...
                    print("✅ [MorningCompletion] Cycle adaptation enabled and snapshot generated")
                }
            } catch {
                print("❌ Error reloading routine: \(error)")
            }
